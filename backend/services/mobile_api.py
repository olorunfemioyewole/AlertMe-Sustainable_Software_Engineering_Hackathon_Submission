"""
SafeAlert Mobile API  (Flutter backend)
JWT-authenticated REST API that proxies incident reports to the core engine.
...
def db_increment_report_and_recalculate(user_id: str) -> None:
    with _conn() as c:
        c.execute("UPDATE users SET report_count = report_count + 1 WHERE id = ?", (user_id,))
        row = c.execute(
        ...
        if row:
            ...
            c.execute("UPDATE users SET credibility_weight = ? WHERE id = ?", (weight, user_id))
        c.commit()
...
async def report_incident(
    ...
    data = resp.json()
    return {"ref_code": data.get("ref_code"), "status": data.get("status", "pending")}
